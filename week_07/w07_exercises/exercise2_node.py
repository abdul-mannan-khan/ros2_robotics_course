#!/usr/bin/env python3
"""
Week 7 Exercise 2: Offboard Commander Node

Waypoint-following offboard control for PX4.

Subscribers: /fmu/out/vehicle_odometry (Odometry)
Publishers:
  /fmu/in/trajectory_setpoint   (PoseStamped)
  /fmu/in/offboard_control_mode (String)
Parameters: waypoints, waypoint_radius (1.0), cruise_speed (2.0)
"""
import json, math
import rclpy
...
class OffboardCommanderNode(Node):
    ...
    def control_loop(self):
        mode_msg = String()
        mode_msg.data = json.dumps({'mode': 'OFFBOARD', 'position': True})
        self.pub_mode.publish(mode_msg)

        if self.current_pose is None or self.current_wp_idx >= len(self.waypoints):
            return

        target = self.waypoints[self.current_wp_idx]

        # TODO: Implement waypoint state machine
        # 1. Get current position: cx, cy, cz = self.current_pose.position.x/y/z
        # 2. Compute distance: dist = sqrt((target[0]-cx)**2 + (target[1]-cy)**2 + (target[2]-cz)**2)
        # 3. If dist < self.wp_radius: advance self.current_wp_idx += 1
        # 4. Publish PoseStamped setpoint to target waypoint
